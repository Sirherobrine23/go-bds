...
func (poggit Poggit) DownloadPhar(resourceId int, fileName string) (io.ReadCloser, error) {
	rootUrl := poggit.Host
	if rootUrl == nil {
		rootUrl = DefaultAPI
	}
	headers := request.Header{}
	res, err := request.MakeRequestWithStatus(rootUrl.ResolveReference(&url.URL{Path: path.Join(rootUrl.Path, strconv.Itoa(resourceId), path.Clean(fileName))}), &request.Options{Header: headers})
	if err != nil {
		return nil, err
