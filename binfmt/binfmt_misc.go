...
func LinuxMiscs() ([]Binary, error) {
	...
	interpreters := []Binary{}
	for _, fileEntry := range mapers.Slice[os.DirEntry](files).Filter(func(input os.DirEntry) bool { return !slices.Contains(miscIgnoreFiles, input.Name()) }) {
		...
		for line := range strings.SplitSeq(strings.TrimSpace(string(miscBuff)), "\n") {
			...
			case "interpreter":
				...
				if mapers.Slice[string](interpreter.Interpreter).Find(func(input string) bool { return strings.Contains(input, "/wine") }) != "" {
